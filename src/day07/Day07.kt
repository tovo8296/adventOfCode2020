...
fun main() {
    ...
    val allowedColors = mutableSetOf<String>()
    findAllowedColors(myColor, allowedColors, rules)
    println("Allowed ${allowedColors.size} colors: $allowedColors")

    val innerBags = countInnerBags(myColor, rules)
    println("Need $innerBags inner bags")
}

private fun countInnerBags(requested: String, rules: Map<String, List<Pair<Int, String>>>): Int {
    var count = 0
    rules[requested]!!.forEach { child ->
        count += child.first
        val subCount = countInnerBags(child.second, rules)
        count += subCount * child.first
    }
    return count
}

private fun findAllowedColors(requested: String, allowedColors: MutableSet<String>, rules: Map<String, List<Pair<Int, String>>>) {
    rules.forEach { entry ->
        entry.value.forEach { child ->
            if (child.second == requested) {
                if (!allowedColors.contains(entry.key)) {
                    allowedColors.add(entry.key)
                    findAllowedColors(entry.key, allowedColors, rules)
                }
            }
        }
    }
...
private fun parse(allRules: String): Map<String, List<Pair<Int, String>>> {
    val map = mutableMapOf<String, List<Pair<Int, String>>>()
    allRules.lines().forEach { line ->
        println("--- process rule: '$line'")
        val parentColor = line.substring(0, line.indexOf(" bags contain"))
        println("Parent color: '$parentColor'")
        if (line.endsWith("contain no other bags.")) {
            map[parentColor] = emptyList()
        } else {
            val children = mutableListOf<Pair<Int, String>>()
            map[parentColor] = children
            val rest = line.substring(line.indexOf("contain ") + "contain ".length)
            val tokenizer = StringTokenizer(rest, ",.")
            while (tokenizer.hasMoreTokens()) {
                val token = tokenizer.nextToken().trim()
                val number = Integer.parseInt(token.substring(0, 1))
                val color = token.substring(2, token.indexOf("bag")).trim()
                println("     $number '$color'")
                children.add(Pair(number, color))
            }
        }
    }
    return map
