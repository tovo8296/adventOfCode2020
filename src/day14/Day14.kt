...
//    runVersion1(program, memory) // part1 => 13496669152158
    runVersion2(program, memory)

    var sum = BigInteger.ZERO
    memory.values.forEach { sum += BigInteger(it.toString()) }
    println("Sum of values: $sum")
}

private fun runVersion1(program: List<Any>, memory: MutableMap<Long, Long>) {
    var bitmask = Array<Char>(36, { bit1Ignore })
    program.forEach { cmd ->
        when (cmd) {
            is Array<*> -> bitmask = cmd as Array<Char>
            is Pair<*, *> -> {
                val index = cmd.first as Int
                val indexL = index.toLong()
                val maskedValue = applyValueBitmask(cmd.second as Long, bitmask)
                if (maskedValue == 0L) {
//                    println("Reset 0 at $index")
                    memory.remove(indexL)
                } else {
//                    println("Store $maskedValue at $index")
                    memory[indexL] = maskedValue
                }
            }
        }
    }
...
private fun runVersion2(program: List<Any>, memory: MutableMap<Long, Long>) {
    var bitmask = Array<Char>(36, { bit2Ignore })
    program.forEach { cmd ->
        when (cmd) {
            is Array<*> -> bitmask = cmd as Array<Char>
            is Pair<*, *> -> {
                val index = (cmd.first as Int)
                val value = cmd.second as Long
                writeMaskedIndex(index, value, memory, bitmask, 0)
            }
        }
    }
}

fun writeMaskedIndex(index: Int, value: Long, memory: MutableMap<Long, Long>, bitmask: Array<Char>, startAt: Int) {
    val nextFloat = findNextFloat(bitmask, startAt)
    if (nextFloat == -1) {
        val maskedIndex = applyIndexBitmask(index.toLong(), bitmask)
        if (value == 0L) {
//                    println("Reset 0 at $index")
            memory.remove(maskedIndex)
        } else {
//                    println("Store $maskedValue at $index")
        ...
        // recursive
        val nextStart = nextFloat + 1
        val bitmaskPos = realizeFloatBit(bitmask, nextFloat, true)
        val bitmaskNeg = realizeFloatBit(bitmask, nextFloat, false)
        writeMaskedIndex(index, value, memory, bitmaskPos, nextStart)
        writeMaskedIndex(index, value, memory, bitmaskNeg, nextStart)
    }
}

fun realizeFloatBit(bitmask: Array<Char>, nextFloat: Any, floatValue: Boolean): Array<Char> {
    return bitmask.mapIndexed { index, c ->
        if (index == nextFloat) {
            require(c == bit2Float)
            if (floatValue) bit2True else bit2False
        } else {
            c
        }
    }.toTypedArray()
...
fun findNextFloat(bitmask: Array<Char>, startAt: Int): Int {
    for (i in startAt until bitmask.size) {
        if (bitmask[i] == bit2Float) {
            return i
        }
    }
    return -1
...
fun applyIndexBitmask(value: Long, bitmask: Array<Char>): Long {
    var maskedValue = value
    bitmask.forEachIndexed { index, bit ->
        val bitIndex = 35 - index
        when (bit) {
            bit2True -> {
                maskedValue = maskedValue or (1L shl bitIndex)
            }
            bit2False -> {
                maskedValue = maskedValue and (-1L xor (1L shl bitIndex))
            }
            bit2Float -> throw IllegalStateException("Try to apply mask with Float: ${bitmask.joinToString("")}")
        }
    }
    return maskedValue
}

fun applyValueBitmask(value: Long, bitmask: Array<Char>): Long {
    var maskedValue = value
    bitmask.forEachIndexed { index, bit ->
        val bitIndex = 35 - index
        when (bit) {
...
fun parse(input: String): List<Any> { // Array<Boolean?> or Pair<Int, Long>
    return input.lines().map { line ->
        if (line.startsWith("mask")) {
            val rest = line.substring("mask = ".length)
            require(rest.length == 36)
            rest.toCharArray().toTypedArray()
        } else if (line.startsWith("mem[")) {
            val match = Regex("mem\\[(\\d+)\\] = (\\d+)").matchEntire(line)!!
            val index = Integer.parseInt(match.groupValues[1])
            val value: Long = java.lang.Long.parseLong(match.groupValues[2])
            Pair(index, value)
        } else {
            throw java.lang.IllegalArgumentException("Unknown line: $line")
        }
    }
}