...
fun main() {
    val source = input
    val foods = parseFoods(source)
    val allergensToIngredients = calcAllergensToIngredientsMap(foods)
    val allIngredients = collectAllIngredients(foods)
    val nonAllergicIngs = calcNonAllergicIngredients(allIngredients, allergensToIngredients)
    countIngredients(nonAllergicIngs, foods)

    checkAllerToIngMap(allergensToIngredients)
    filterAllergensMapping(allergensToIngredients)
    checkAllerToIngMap(allergensToIngredients)

    println("Dangerous Ingredients")
    println(allergensToIngredients.keys.sorted().map { allergensToIngredients[it]!!.first() }.joinToString(","))
    // nfnfk,nbgklf,clvr,fttbhdr,qjxxpr,hdsm,sjhds,xchzh
}

fun filterAllergensMapping(allergensToIngredients: MutableMap<String, Set<String>>) {
    val filteredAllergens = mutableSetOf<String>()
    var changed = false
    do {
        changed = filterAllergensMappingOnce(allergensToIngredients, filteredAllergens)
    } while (changed)
}

private fun filterAllergensMappingOnce(allergensToIngredients: MutableMap<String, Set<String>>, filteredAllergens: MutableSet<String>): Boolean {
    allergensToIngredients.keys.sorted().forEach { singleAller ->
        val singleIngs = allergensToIngredients[singleAller]!!
        if (!filteredAllergens.contains(singleAller) && singleIngs.size == 1) {
            val singleIng = singleIngs.first()
            println("Filter: $singleAller is only in $singleIng")
            filteredAllergens.add(singleAller)
            allergensToIngredients.keys.toList().forEach { aller ->
                if (aller != singleAller) {
                    allergensToIngredients[aller] = allergensToIngredients[aller]!!.filter { it != singleIng }.toSet()
                }
            }
            return true
        }
...
fun checkAllerToIngMap(allergensToIngredients: MutableMap<String, Set<String>>) {
    println("\n Check Allergens: ${allergensToIngredients.keys.sorted().toList()}")
    allergensToIngredients.keys.sorted().forEach { aller ->
        val ings = allergensToIngredients[aller]!!
        if (ings.size != 1) {
            println("Warning: $aller can be in $ings")
        }
    }
}

fun countIngredients(ingredients: List<String>, foods: List<Food>) {
    var count = 0
    foods.forEach { food ->
    ...
    println("Ingredient Count: $count")

}

fun calcNonAllergicIngredients(allIngredients: Set<String>, allergensToIngredients: MutableMap<String, Set<String>>): List<String> {
    return allIngredients.filter { ing ->
        allergensToIngredients.values.none { ingList ->
            ingList.contains(ing)
        }
    }

}

fun collectAllIngredients(foods: List<Food>): Set<String> {
    val all = mutableSetOf<String>()
    foods.forEach { food ->
        all.addAll(food.ingredients)
    }
    return all
}

fun calcAllergensToIngredientsMap(foods: List<Food>): MutableMap<String, Set<String>> {
    val map = mutableMapOf<String, Set<String>>()
    foods.forEach { food ->
        println("Cal for $food")
        food.allergens.forEach { allergen ->
            if (map.containsKey(allergen)) {
                val ingsOfAller = map[allergen]!!
                val filtered = ingsOfAller.filter { ing -> food.ingredients.contains(ing) }
                map[allergen] = filtered.toSet()
            } else {
                map[allergen] = food.ingredients.toSet()
            }
        }
    }
    return map
...
fun parseFoods(source: String): List<Food> {
    val sepText = "(contains"
    return source.lines().map { line ->
        val sep = line.indexOf(sepText)
        val ingredients = StringTokenizer(line.substring(0, sep), " ").toList().map { it.toString() }
        val allergens = StringTokenizer(line.substring(sep + sepText.length, line.length - 1), ",").toList().map { it.toString().trim() }
        Food(ingredients, allergens).also {
            //            println(it)
        }
    }
}
