...
enum class PassportKeys(name: String) {
    byr("byr"),
    iyr("iyr"),
    eyr("eyr"),
    hgt("hgt"),
    ...
    fun isValid(value: String): Boolean = when(this) {
        byr -> Regex("\\d\\d\\d\\d").matchEntire(value) != null &&
                Integer.parseInt(value).let { it in 1920..2002 }
        iyr -> Regex("\\d\\d\\d\\d").matchEntire(value) != null &&
                Integer.parseInt(value).let { it in 2010..2020 }
        eyr -> Regex("\\d\\d\\d\\d").matchEntire(value) != null &&
                Integer.parseInt(value).let { it in 2020..2030 }
        hgt -> Regex("\\d+(cm|in)").matchEntire(value) != null &&
                Integer.parseInt(value.substring(0, value.length - 2)).let {
...
fun main() {
    ...
    val fakeValidCount = passports.count { pp ->
        pp.containsKey(PassportKeys.byr) &&
                pp.containsKey(PassportKeys.iyr) &&
                pp.containsKey(PassportKeys.eyr) &&
                pp.containsKey(PassportKeys.hgt) &&
                pp.containsKey(PassportKeys.hcl) &&
                pp.containsKey(PassportKeys.ecl) &&
                pp.containsKey(PassportKeys.pid) &&
                pp.keys.all {key ->
                    val value = pp.getValue(key)
                    key.isValid(value)
                }
    }
    println("Valid passports: $fakeValidCount")
}

fun parseInput(input: String): List<Map<PassportKeys, String>> {
    val result = mutableListOf<Map<PassportKeys, String>>()

    var map = mutableMapOf<PassportKeys, String>()
    result.add(map)
    input.lines().forEach { line ->
        if (line.trim().length == 0){
            map = mutableMapOf<PassportKeys, String>()
            result.add(map)
        }else {
            val tokenizer = StringTokenizer(line.trim())
            while(tokenizer.hasMoreTokens()) {
                val token = tokenizer.nextToken()
                val sep = token.indexOf(':')
                val keyName = token.substring(0, sep)
                val key = PassportKeys.valueOf(keyName)
                val value = token.substring(sep + 1)
                map.put(key, value)
            }
        }
    }
    println("\nFound ${result.size} passports")
    return result
}
