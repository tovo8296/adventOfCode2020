...
fun main() {
    var yesAnswers = mutableMapOf<Char, Int>()
    var groupSize = 0
    input.lines().forEach { line ->
        if (line.trim().isEmpty()) {
            groupFinished(yesAnswers, groupSize)
            yesAnswers.clear()
            groupSize = 0
        } else {
            line.forEach { c ->
                val count = yesAnswers.get(c) ?: 0
                yesAnswers.put(c, count + 1)
            }
            groupSize++
        }
    }
...
private fun groupFinished(yesAnswers: MutableMap<Char, Int>, groupSize: Int) {
    var groupYesCount = yesAnswers.count { it.value == groupSize }
    println("--- Group has $groupYesCount yes questions")
    totalCount += groupYesCount
}
